...
pub trait RedisORM {
    fn save(&self,con:&mut redis::Connection) -> RedisResult<()> where Self: Serialize{
        default_save(con, &Self::redis_type_name(), &self.redis_id(), self)
    }
    fn fetch_from_uuid( con: & mut redis::Connection, id: &str) -> Option<Self> where Self: Sized, for<'a> Self: Deserialize<'a>, Self: Debug{
        default_fetch_from_uuid(con, Self::redis_type_name().as_str(), id)
    }
    fn example() -> Self where Self: Sized;
    fn all(con: &mut redis::Connection) -> Vec<Self> where Self: Sized, for<'a> Self: Deserialize<'a>, Self: Debug{
