...
async fn authenticated(claims: OidcClaims<EmptyAdditionalClaims>,id: Path<String>) -> impl IntoResponse {
    format!("Hello {} {:?} {:?}", claims.subject().as_str(),claims.email().expect("mail broken"),claims.preferred_username().expect("username broken"))
}
